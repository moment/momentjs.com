...
function concatenate(sources, destination, transform) {
	const content = sources.map(function (source) {
...
function checkLibraryVersion(name) {
	...
	if (!fs.existsSync(libraryPackage)) {
		throw new Error('Missing ' + name + ' submodule. Run `git submodule update --init`.');
	}
	...
	if (actual !== expected) {
		throw new Error(
			name + ' submodule is ' + actual + ', but the installed npm package is ' + expected + '.'
		);
	}
}
...
module.exports = async function () {
	...
	const timezoneTests = [
		...
		...filesBelow('libs/moment-timezone/tests/helpers').filter(file => file.endsWith('.js')),
		...filesBelow('libs/moment-timezone/tests/zones').filter(file => file.endsWith('.js')),
		...filesBelow('libs/moment-timezone/tests/moment-timezone').filter(file => file.endsWith('.js')),
		'assets/js/test-nodeunit.js',
		'assets/js/timezone-test-end.js'
	];

	concatenate(timezoneTests, 'build/static/js/timezone-test.js', function (source, filename) {
	...
	]).process(css, { from: 'assets/css/style.scss', to: 'build/static/css/style.css' });
	...
	const downloads = {
		...
		'moment-timezone.min.js': 'libs/moment-timezone/builds/moment-timezone.min.js',
		'moment-timezone-with-data.js': 'libs/moment-timezone/builds/moment-timezone-with-data.js',
		'moment-timezone-with-data.min.js': 'libs/moment-timezone/builds/moment-timezone-with-data.min.js',
		'moment-timezone-with-data-1970-2030.js': 'libs/moment-timezone/builds/moment-timezone-with-data-1970-2030.js',
		'moment-timezone-with-data-1970-2030.min.js': 'libs/moment-timezone/builds/moment-timezone-with-data-1970-2030.min.js',
		'moment-timezone-with-data-10-year-range.js': 'libs/moment-timezone/builds/moment-timezone-with-data-10-year-range.js',
		'moment-timezone-with-data-10-year-range.min.js': 'libs/moment-timezone/builds/moment-timezone-with-data-10-year-range.min.js'
	};
	...
	copy('libs/moment-timezone/data/unpacked/latest.json', 'build/data/moment-timezone-unpacked.json');
	copy('libs/moment-timezone/data/meta/latest.json', 'build/data/moment-timezone-meta.json');
};
