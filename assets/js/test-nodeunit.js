...
(function () {
  ...
  function updateTotals(_passed, _failed) {
    ...
    banner.html(
      '<span class="test-num">' +
        _passed +
        "</span> tests passed<br/>" +
        '<span class="fail-label"><span class="test-num">' +
        _failed +
        "</span> failed</span>"
    );
  }
  ...
  (function () {
    ...
    for (i = 0; i < pieces.length; ++i) {
      ...
      queryArgs[decodeURIComponent(key_val[0])] = decodeURIComponent(
        key_val[1]
      );
    }
  ...
  nodeunit.runModules(exports, {
    moduleStart: function (name) {
      ...
      currentModuleEl.append(
        '<h3 class="tests-module-title">' + name + "</h3>"
      );
    ...
    testDone: function (name, assertions) {
      ...
      // each test
      testHtml +=
        '<div class="tests-test-title"><strong>' + name + "</strong> ";
      ...
      for (var i = 0; i < assertions.length; i++) {
        ...
        if (assert.failed()) {
          assertHtml +=
            ...
            ")";
          assertHtml +=
            "<pre>" + (assert.error.stack || assert.error) + "</pre>";
      ...
      if (failures) {
        ...
        reportHTML +=
          "<p>It's hard to catch bugs across all browsers and timezones. If you have a minute, please report the failing test.</p>";
        reportHTML +=
          "<a class='button' href='" +
          searchUrl +
          "' target='_blank'><b>STEP 1:</b> Search for an existing failure report</a>";
        reportHTML +=
          "<p>If it doesn't look like this failure has already been reported, proceed to step 2.</p>";
        reportHTML +=
          "<a class='button' href='" +
          submitUrl +
          "' target='_blank'><b>STEP 2:</b> Submit a failure report</a>";
        ...
        reportHTML +=
          "<p class='success'>Awesome, all the unit tests passed!</p>";
      }

      $("#report-wrapper").html(
        '<div class="tests-reporting">' + reportHTML + "<div>"
      );
